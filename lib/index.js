...
function getRandomPhoto( msg, sites, callback ) {
	...
	console.log( '------\nSite: ' + site + '\n' );
	var url = getURL( blog, function( url ) { 
		msg.reply( url );
	});
};
...
function getURL( blog, offset, callback ) {
	...
	var offset = 0;
	blog.photo( {type: 'photo', limit: '1'}, function( error, response, callback ) {
		...
		offset = Math.floor( Math.random() * response.total_posts );
		
		console.log( 'Offset: ' + offset + '\n');
		
		blog.photo( {type: 'photo', limit: '1', offset: offset}, function(error, response, callback) {
